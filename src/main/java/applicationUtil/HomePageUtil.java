...
public class HomePageUtil {
	...
	public boolean VerifyListOfCourse(WebDriver driver) {
		...
		try {
			...
			for(int i=0;i<2;i++) {
				...
				result = coursePageUtilObj.verifyClickBuy();
				if (!result) {
					return result;
				}
				
				result = coursePageUtilObj.verifyPackages();
				if (!result) {
					return result;
				}
				result = coursePageUtilObj.verifyClickOfferPromo("Platinum");
				if (!result) {
					return result;
				}
				cfObj.commonClick(homePageORObj.getCloseButton());
	...
	public boolean clickOnMyLibraryButton(WebDriver driver) {
		...
		try {
			...
			if(!result)
			{
				...
				result=cfObj.commonWaitForElementToBeVisible(driver,homePageORObj.getLibraryExplore_Courses() , 1);
				if(result==true) {
	...
	public boolean clickOnMyprofileButton(WebDriver driver) {
		...
		try {
			...
			if(!result)
			{
				homePageMsgList.add("Not Verify Input data in Profile Page");
        		return result;
			}
			cfObj.commonClick(homePageORObj.getHomePage());
